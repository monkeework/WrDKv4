...
if(isset($_GET['act'])){$gender = $_GET['act'];}else{ $gender = '';}
...
{//check 'act' for type of process
	case "m":
		...
		echo "<h3>Current Unclaimed <b>Male</b> Playbys:</h3>
	...
	#race = r (white, black, asain, hispanic, mixed)
		echo showAvailable($gender, $b='', $e='', $h='', $s='');

		break;
	########################################################
	case "f":
		#show available male casting options.
		echo "<h3>Current Unclaimed <b>Female</b> Playbys:</h3>
			<div class='clearfix'><div>";
		echo showAvailable($gender, $b='', $e='', $h='', $s='');
	...
	default:
		...
		if( (isset($_REQUEST['act'])) && ($_REQUEST['act']) ){
			...
			$pbSearch = str_replace(" ", "_", $pbName);
			#echo 'search 2 ' . $pbSearch  . '<br />';
			$pbSearch = str_replace("+", "_", $pbSearch);
			#echo 'search 2 ' . $pbSearch  . '<br />';
		...
		if((!isset($_REQUEST['act'])) || (empty($_REQUEST['act']))){
			...
			echo $aarGeneral['uploadDefault']; #Default content
		}
...
function showAvailable($gender, $str = ''){
/**
	* Show unclaimed PBs
	*
	*
