...
function gt_charIndex($str ='')
{
 ...
 /* IS THIS GOING TO BE SELF-PUNISHING AFTER A WHILE?

 /* returns the CharID of last person to post in a thread */
 function gt_last2post(){
...
/**
	...
	*/
function mk_postOrder($tID, $aParticipants, $alastPosting, $last2post=0, $str=''){
...
#dumpDie($alastPosting);
	 ...
	 foreach ($alastPosting as $arrData){
         #get ThreadID from $alastPosting
         $tMatch = $arrData['ThreadID'];
         #we get the matching value of the array
		 ...
		 if ($tMatch == $tID){
			 ...
			 (int)$tempCharID = $arrData['CharID'];
			 (int)$tempMaxID  = $arrData['MaxPostID']; #last post character posted in
	...
	if(count($aParticipants) > 0){
		...
		foreach($aParticipants as $charID) {
			...
			if(($charID == $last2post) && ($tempCharID == $charID)){
...
//POST ORDER begin
                    ...
                    /**
                     * GOAL - great a list which shows the order in which characters joined the thread, hightlight the last character to appear or bold them.
                     */
...
#var_dump($tTagIndex);
	...
	// go to thread link
	$str .=  '<a class="pull-right" href="' . $tURL . '"> <span class="glyphicon glyphicon-share"></span> SOMEONE </i></a></p>
			</div>
		</div>
	</div>';
}
