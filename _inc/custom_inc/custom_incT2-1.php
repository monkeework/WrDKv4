...
/*
 * get cID of last character to post in a thread
 */

/*
 * ATTEMPT ONE
 function gt_last2post($aLastPost=[]){
...
#ATTEMPT TWO
function gt_last2post($tID, $threadID=0, $Last2Post=''){
    #pulls back CharID of last char to post in a specific thread

    /*
     *  Error in query (1064): Syntax error near '== 20' at line 6
     *
        Attempted to filter sql to only deliver the specific thread
        SELECT `ThreadID`, `CharID`, `PostID`
            FROM `ma_Posts`
            WHERE `PostTags` IS NOT NULL
                AND `PostTags` != '0'
                AND `PostTags` != ' '
                AND `ThreadID` == 20;
     */
    $sql = "SELECT `ThreadID`, `CharID`, `PostID` 
    FROM `ma_Posts` 
    WHERE `PostTags` IS NOT NULL 
        AND `PostTags` != '0' 
        AND `PostTags` != ' ';";

    $db = pdo($sql); # pdo() creates and returns a PDO object
##comes back as object(PDO)[6] - want int of 60
#dumpDie($db);
    #comes back as object(PDO)[6] - want int of 60
    #$result stores data object in memory
    try {$result = $db->query($sql);} catch(PDOException $ex) {trigger_error($ex->getMessage(), E_USER_ERROR);}
        ...
        if($result->rowCount() > 0){#there are records - present data
            while($row = $result->fetch(PDO::FETCH_ASSOC))
            {# pull data from associative array
                $cID = ($row['CharID']);
                $threadID == ($row['CharID']);
...
/**
 ...
 function mk_postOrder($tID, $aParticipants, $alastPosting, $last2post='',  $str=''){
    ...
    #dumpDie($alastPosting);
     ...
     #string '' (length=0) = '';
     foreach ($alastPosting as $key => $value) {
         if($tID == $value){
             $alastPosting[0]['CharID'] = $str;
         }
     }
	...
	if(count($aParticipants) > 0){
		...
		foreach($aParticipants as $charID) {
			...
			if($charID == $last2post){
			    $str .= '<a href="' . VIRTUAL_PATH . 'characters/profile.php?CodeName=' . $codeName . '&id=' . $charID . '&act=show" title="Profile for ' . $str . '"><strong>' . $codeName . '</strong></a>, ';
			}else{
...
function mk_cFeaturedLinks($threadID, $aPosts, $aChars=[], $aParticipants=[], $postParticipantID=0, $str='') {
 ...
 //ThreadRecnt begin delivers base page
 //ThreadRecnt begin
 //ThreadRecnt begin
...
#dumpDie($aCharNameIndex);
		...
		$alastPosting = gt_last2post($tID);

		dumpDie($alastPosting);
#dumpDie($alastPosting);
// see custom_inc-vDump-aLastPosting.png
