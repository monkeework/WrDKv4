...
function mk_cFeaturedLinks($tID, $aCharacterBank, $arr=[], $cName=[], $count = 1, $str='') {
	#$ThreadId
	...
	foreach($aCharacterBank[0] as $data) {
		...
		#array to store the unique values with
		$aSort = [];
		...
		if($data['ThreadID'] == $tID) {
			...
			if($cID){
				#save character ids to array
				$arr[] = $_SESSION['charIndex'][$cID];

				#check whats going on
				$testCharName = $_SESSION['charIndex'][$cID];
				...
				echo '<a href="./profile.php?act=show&charID=' . $testCharId . '" title="' . $testCharName . '" >' . $testCharName . '</a><br> ';
				#$str .=  '<a href="./profile.php?act=show&charID=' . $testCharId . '" title="' . $testCharName . '" >' . $testCharName . '</a>,';
