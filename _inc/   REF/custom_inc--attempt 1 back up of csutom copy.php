...
function mk_cFeaturedLinks($threadID, $aCharacterBank, $aCharID=[], $str='') {
	#$ThreadId
	...
	$characterID = $characterName = $testForEchoes ='';
	foreach($aCharacterBank[0] as $data) {
		...
		if($data['ThreadID'] == $threadID) {
			#make character id
			$characterID = $data['CharID']; #get character id

			if($characterID > 0){

				#convert charater int to string name of character
				$characterName = $_SESSION['charIndex'][$characterID];
			...
			#var_dump($aCharID);
			#clean array of non unique ings
			$aCharID = array_unique($aCharID);
			#var_dump($aCharID);
			...
			if($characterID > 0 ){
				...
				$testForEchoes = $characterName . ' ( ' . $characterID  . ' ) ';
				if( $count > 0 ){ $testForEchoes = ', ' . $testForEchoes;}
				echo $testForEchoes;


				$str .=  '<a href=profile.php?act=show&charID="'
				. $characterID . '" title="'
				. $characterID . ' > '
				. $characterName .' </a>';
...
$aCharacterBank[] = mk_charIndex();
 $str .= mk_cFeaturedLinks(20, $aCharacterBank);
...
#var_dump($_SESSION);
					...
					#get featured character list, sorted
...
#var_dump($aCharacterBank);
				...
				$str .= 'FAKE - placeholder Chimeara<br/>
								FAKE <br/>
								FAKEH<br/>
								FAKE Also<br/>';
