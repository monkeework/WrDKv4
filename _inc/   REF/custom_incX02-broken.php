...
#?act=postEdit&uID=1&cat=3&tID=9&pID=0059&ttl=OOC_TT#2&typ=OOC&pOdr=1&rtg=G_-_General_Audiences&wet=&phs=Midday&tme=01/22/2017_02:03:29_pm&loc=Avengers_Mansion&whn=when
...
pOdr=1
&
rtg=G_-_General_Audiences
&
wet=
&
phs=Midday
&
tme=01/22/2017_02:03:29_pm
&
...
function mk_cFeaturedLinks($tID, $aCharacterBank, $str='')
{
	#$ThreadId
	#dumpDie($aCharacterBank);
	#dumpDie($_SESSION['characters']);
	foreach($aCharacterBank[0] as $data) {

		#ultimately remove these, these are for test purposes only
		$testCharName=$testCharId='';


		if($data['ThreadID'] == $tID) {

			#make character id
			$testChaID = $aCharacterBank[0]['CharID'];

			#ID exists? Must be higher than zero
			if($testChaID > 0){

				#test first

				#deal with the name
				#check that we have the character name
				#echo $_SESSION['charIndex'][$testCharId];
				#$testCharName = $_SESSION['charIndex'][$testCharId];

#Do we need this now?
#save character name to array
#$cName[] = $_SESSION['charIndex'][$cID];
#save character name to var to try another way to return data
#$aCharName = $_SESSION['charIndex'][$cID];

				#array_unique($a)
				#check whats going on
				#$_SESSION['charIndex'][$cID];
				echo $testCharName . '(' . $testChaID . ') <br />';

				echo $testChaID . '<br />';

				#echo '<a href="./profile.php?act=show&charID=' . $testChaID . '" title="' . $testCharName . '" >' . $testCharName . '</a><br> ';

				#$str .= '<a href="./profile.php?act=show&charID=' . $testChaID . '" title="' . $testCharName . '" >' . $testCharName . '</a><br> ';

			}


			#var_dump($str);
			#retun all links of thread partipants
			return $str;

/*
			#HAVE =   3        4          15       1         3         60        3        60       60
			#HAVE = Wiccan - banshee - Sunfire - Cyclops - Wiccan - Ulysses - Wiccan - Ulysses - Ulysses

			#WANT =   3        4          15       1         					 60      // unigue/distinct values only
			#WANT = Wiccan - banshee - Sunfire - Cyclops      -     Ulysses    // unigue/distinct values only

			#remove duplicates
			#make unigue - manes and id's should matchup


			#MAKE = #WANT =   3   4       15       1                   60      // unigue/distinct values only
			array_unique($arr);
			#MAKE = Wiccan - banshee - Sunfire - Cyclops      -     Ulysses    // unigue/distinct values only
			array_unique($cName);


			#bring it together in returned string of links
			foreach ($arr as $id) {


																									#ID RETURNING NULL      #cName RETURNING Undefined offset: 1
				$str .= '<a href="./profile.php?act=show&charID=' . $id . '" title=" $cName[$count] " > $cName[$count] </a>';

				#non name repeating link - instead of clyclops cylcops
				#make <a href="./profile.php?act=show&charID=1" title="Cyclops" > Cyclops </a>

				#coount defined in function params()
				$count++;
			}
*/
...
function threadRecent($sql, $sqlTags, $tId='',$str=''){
	/**
		...
		<div class="bs-example">
			...
			{//show results
				...
				{# process each row
					...
					$aCharacterBank[] = mk_charIndex();

#var_dump($aCharacterBank);
#var_dump($_SESSION);

#$charFeatured   = mk_cFeaturedLinks($tID, $aCharacterBank );


#var_dump($charFeatured);
#var_dump($aCharacterBank);
					...
					$str .=  '<p class="col-md-9">'. $row['ThreadSummary'] . '</p>
								...
								<br/>
								Chimeara<br/>
								Anansi<br/> Human Torch<br/>
								Chant<br/>';


								#get thread participants
								$str .= mk_cFeaturedLinks($tID, $aCharacterBank);
...
function postInsert($str=''){
	...
	*/
	...
	$stmt->bindValue(5,		$pOrder 				,	PDO::PARAM_STR);

	$stmt->bindValue(6,		$pRating 				,	PDO::PARAM_STR);
	$stmt->bindValue(7,		$pFeaturing 		,	PDO::PARAM_STR);
	$stmt->bindValue(8,		$pPhase					,	PDO::PARAM_STR);
	$stmt->bindValue(9,		$postTimeOfDay 	,	PDO::PARAM_STR);
	$stmt->bindValue(10,	$pWeather 			,	PDO::PARAM_STR);
	$stmt->bindValue(11,	$pLocation 			,	PDO::PARAM_STR);
	$stmt->bindValue(12,	$pContent 		  ,	PDO::PARAM_STR);
	$stmt->bindValue(13,	$pNotes 				,	PDO::PARAM_STR);
	$stmt->bindValue(14,	$pSummary 			,	PDO::PARAM_STR);
	$stmt->bindValue(15,	$pTags 					,	PDO::PARAM_STR);
...
Failed to parse time string (1978-23-9) at position 6 (3):
Unexpected character' in /home/maxster/marvel-champions.com/_inc/custom_inc.php:

5330 Stack trace: #0 /home/maxster/marvel-champions.com/_inc/custom_inc.php(5330):
DateTime->__construct('1978-23-9')

#1 /home/maxster/marvel-champions.com/_inc/custom_inc.php(5353): setAge('1978-23-9')

#2 /home/maxster/marvel-champions.com/characters/profile.php(371):
showAge('date of birth', '1978-23-9', '0', '1978', '23', '9')

#3 {main} thrown in /home/maxster/marvel-champions.com/_inc/custom_inc.php on line 5330
