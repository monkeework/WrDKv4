...
echo '<div class="container-fluid main-container">
	...
	$str='';
	$cID=$cName='';
	$formatPostID=$pID=$postLocation=$postWeather=$postContent=$postNotes=$postSummary=$postTags='';
	$tID='';
	...
	$sqlTags = "SELECT CharID AS CharacterID, Codename, FirstName, LastName FROM ma_Characters;";
	#sql for post data
	$sqlPost = "SELECT * FROM ma_Posts ORDER BY DateCreated";
	...
	$pager = new Pager(2,'',$prev,$next,'');
	$sql = $pager->loadSQL($sqlPost);  #load SQL, add offset
			...
			*/
			...
			<div class="col-md-9 pull-right">';
				...
				{# process each row
					...
					$pID   						= (int)$row['PostID'];
					$formatPostID  		= str_pad($pID, 4, '0', STR_PAD_LEFT);
					...
					#Polygon no workie with safari :(
					$str .= '<div class="row">
						<div class="col-sm-2">' . mk_cThumb($cID) . '</div>'; #gets thumbnail of featured character with detailed tooltip

					#get the post content - the actual meat of the post
					$str .= '<div class="col-sm-7" style="border: 1px solid red; color: #888">
							<p><strong>' . $cName . ' &raquo; </strong> ' .  nl2br($postContent) . '</p>
						</div>
						...
						<div class="col-sm-3 small">
							<div class="well">
								<p style="border: 1px solid red; color: #888"><strong>Message '
								...
								<p><strong>Updated &raquo; </strong> ' . $gDate . '</p>';


								$str .=	'<p>' . getHandlerDetails($hID, $pID) . '<p>'; #returns a handler detail link
	...
	$str .= '
		<div><a href="' . VIRTUAL_PATH . 'characters/profile.php?id=' . $cID . '&act=show"
			...
			>
				...
				<div class="">
					<img src="' . $filename . '" alt="' . $cName. '" class="polygon-clip-hexagon">
				</div>
		</a></div>

		<div>
			<p class="text-center"><small><strong>' . $cName . '</strong></small></p>
		</div>';
...
function getPolyThumb($cID, $str=''){
	...
	#construct the polyThumb
	$str .= '
		<div><a href="' . VIRTUAL_PATH . 'characters/profile.php?id=' . $charID . '&act=show"
			target="_blank" data-toggle="tooltip" data-placement="right" title="" data-container="body" class="tooltiplink" data-html="true"
				...
				*/
				...
				if($myThreadID == $threadID){
					...
					<div class="col-sm-3 small" >
						...
						$str .=	getHandlerDetails($hID, $pID); #returns a handler detail link
