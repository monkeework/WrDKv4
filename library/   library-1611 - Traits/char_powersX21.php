...
function maxDoc_library_char_powers(){
	/**
...
get_header('headerJumbo-inc.php', 'bgPowers01.jpg', 'Powers');

#echo MaxNotes($pageDeets); #notes to me!


#$baseSQL .= $cat == Descriptions!
$baseSQL = "SELECT PowID, PowName, PowDesc, Available, LastUpdated  FROM `ma_CharPower` WHERE CatID ";

switch ($myAction){//check 'act' for type of process

	case 'combat-powers':#final setup of sql
			$cat  = 'combat';
			$sql  = $baseSQL . "= '$cat'; "; //for btns

			echo genOverview($myAction, $aarLibrary_powers['powersCombat'], $codeName, $charID, $gender);
			echo genCategoryBtns($cat, $codeName);
			echo genCategory($sql, $cat, $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case 'defensive-powers':#final setup of sql
			$cat  = 'defensive';
			$sql  = $baseSQL . "= '$cat'; "; //for btns

			echo genOverview($myAction, $aarLibrary_powers['powersDefensive'], $codeName, $charID, $gender);
			echo genCategoryBtns($cat, $codeName);
			echo genCategory($sql, $cat, $charID, $codeName, $gender, $stageID);
		break;
	########################################################
	case "detection-powers":#final setup of sql
			$cat  = 'detection';
			$sql  = $baseSQL . "= '$cat'; "; //for btns

			echo genOverview($myAction, $aarLibrary_powers['powersDetection'], $codeName, $charID, $gender);
			echo genCategoryBtns($cat, $codeName);
			echo genCategory($sql, $cat, $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case "energy-control-powers":#final setup of sql
			$cat  = 'energy control';
			$sql  = $baseSQL . "= '$cat'; "; //for btns

			echo genOverview($myAction, $aarLibrary_powers['powersEnergyCon'], $codeName, $charID, $gender);
			echo genCategoryBtns($cat, $codeName);
			echo genCategory($sql, $cat, $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case "energy-emission-powers":#final setup of sql
			$cat  = 'energy emission';
			$sql  = $baseSQL . "= '$cat'; "; //for btns

			echo genOverview($myAction, $aarLibrary_powers['powersEnergyEm'], $codeName, $charID, $gender);
			echo genCategoryBtns($cat, $codeName);
			echo genCategory($sql, $cat, $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case "faith-powers":#final setup of sql
			$cat  = 'faith';
			$sql  = $baseSQL . "= '$cat'; "; //for btns

			echo genOverview($myAction, $aarLibrary_powers['powersFaith'], $codeName, $charID, $gender);
			echo genCategoryBtns($cat, $codeName);
			echo genCategory($sql, $cat, $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case "illusion-powers":#final setup of sql
			$cat  = 'illusions';
			$sql  = $baseSQL . "= '$cat'; "; //for btns

			echo genOverview($myAction, $aarLibrary_powers['powersillusions'], $codeName, $charID, $gender);
			echo genCategoryBtns($cat, $codeName);
			echo genCategory($sql, $cat, $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case "lifeform-control-powers":#final setup of sql
			$cat  = 'life form control';
			$sql  = $baseSQL . "= '$cat'; "; //for btns

			echo genOverview($myAction, $aarLibrary_powers['powersLifeformCon'], $codeName, $charID, $gender);
			echo genCategoryBtns($cat, $codeName);
			echo genCategory($sql, $cat, $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case "magic-powers":#final setup of sql
			$cat  = 'magical';
			$sql  = $baseSQL . "= '$cat'; "; //for btns

			echo genOverview($myAction, $aarLibrary_powers['powersMagical'], $codeName, $charID, $gender);
			echo genCategoryBtns($cat, $codeName);
			echo genCategory($sql, $cat, $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case "matter-control-powers":#final setup of sql
			$cat  = 'matter control';
			$sql  = $baseSQL . "= '$cat'; "; //for btns

			echo genOverview($myAction, $aarLibrary_powers['powersMatterCon'], $codeName, $charID, $gender);
			echo genCategoryBtns($cat, $codeName);
			echo genCategory($sql, $cat, $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case "matter-conversion-powers":#final setup of sql
			$cat  = 'matter conversion';
			$sql  = $baseSQL . "= '$cat'; "; //for btns

			echo genOverview($myAction, $aarLibrary_powers['powersMatterConversion'], $codeName, $charID, $gender);
			echo genCategoryBtns($cat, $codeName);
			echo genCategory($sql, $cat, $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case "matter-creation-powers":#final setup of sql
			$cat  = 'matter creation';
			$sql  = $baseSQL . "= '$cat'; "; //for btns

			echo genOverview($myAction, $aarLibrary_powers['powersMatterCreation'], $codeName, $charID, $gender);
			echo genCategoryBtns($cat, $codeName);
			echo genCategory($sql, $cat, $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case "mental-enhancement-powers":#final setup of sql
			$cat  = 'mental enhancement';
			$sql  = $baseSQL . "= '$cat'; "; //for btns

			echo genOverview($myAction, $aarLibrary_powers['powersMental'], $codeName, $charID, $gender);
			echo genCategoryBtns($cat, $codeName);
			echo genCategory($sql, $cat, $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case "physical-enhancement-powers":#final setup of sql
			$cat  = 'physical enhancement';
			$sql  = $baseSQL . "= '$cat'; "; //for btns

			echo genOverview($myAction, $aarLibrary_powers['powersPhysical'], $codeName, $charID, $gender);
			echo genCategoryBtns($cat, $codeName);
			echo genCategory($sql, $cat, $charID, $codeName, $gender, $stageID);
		break;
	########################################################
	case "restricted-powers":#final setup of sql
			$cat  = 'restricted';
			$sql  = $baseSQL . "= '$cat'; "; //for btns

			echo genOverview($myAction, $aarLibrary_powers['powersRestricted'], $codeName, $charID, $gender);
			echo genCategoryBtns($cat, $codeName);
			echo genCategory($sql, $cat, $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case "self-alteration-powers":#final setup of sql
			$cat  = 'self-alteration';
			$sql  = $baseSQL . "= '$cat'; "; //for btns

			echo genOverview($myAction, $aarLibrary_powers['powersSelfAlt'], $codeName, $charID, $gender);
			echo genCategoryBtns($cat, $codeName);
			echo genCategory($sql, $cat, $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case "travel-powers":#final setup of sql
			$cat  = 'travel';
			$sql  = $baseSQL . "= '$cat'; "; //for btns

			echo genOverview($myAction, $aarLibrary_powers['powersTravel'], $codeName, $charID, $gender);
			echo genCategoryBtns($cat, $codeName);
			echo genCategory($sql, $cat, $charID, $codeName, $gender, $stageID);
...
*/
	...
	default:
		#customize category
		$myBody = $aarLibrary_powers['powersOverview'];
...
function genOverview($myAction = '', $mainContent = '', $codeName, $charID, $gender, $str=''){ #format descriptions from array and
	...
	$str .= personalizeStr();
	$str .= genLegend($codeName, $charID, $gender);
...
function genLegend($codeName = '', $charID = 0, $gender = '',  $str='' ){ #generate category legend and links from array
	...
	$myLoadedQuery = [
			'combat-powers', 'defensive-powers', 'detection-powers', 'energy-control-powers', 'energy-emission-powers', 'faith-powers', 'illusion-powers', 'lifeform-control-powers', 'magic-powers', 'matter-control-powers', 'matter-conversion-powers', 'mental-enhancement-powers', 'restricted-powers', 'self-alteration-powers', 'travel-powers'
		];
	...
	foreach($myTitles as $title){
		...
		$act = str_replace("/WrDK/traits/char_powers.php?act=","", $chek );
		...
		if($act == $url){
				$str .= '<li class="active">
					<a style="color: white;" href="'
						. VIRTUAL_PATH . 'library/char_powers.php?act='
						. $myLoadedQuery[$count++]
				...
				$str .= '<li>
					<a class="" href="' . VIRTUAL_PATH . 'library/char_powers.php?'
						. '&codeName=' . $codeName
						. '&charID=' . $charID
						. '&gender=' . $gender
						. '">'
			...
			#show me unhighlighted all others
				...
				$str .= '<li>
					<a  href="' . VIRTUAL_PATH . 'library/char_powers.php?act='
						. $myLoadedQuery[$count++]
						. '&codeName=' . $codeName
						. '&charID=' . $charID
						. '&gender=' . $gender
...
#INCOMING -> #http://localhost/WrDKv3/library/char_powers.php?act=combat-powers&charID=60&codeName=Ulli&statusID=5&gender=male
#genCategory('defensive', $CodeName, $CharID, $Gender='')
#act=combat-powers&codeName=&charID=0&codeName=Chimeara&charID=55&statusID=4&gender=male
#http://localhost/WrDKv3/library/char_powers.php?act=combat-powers&codeName=Havok&charID=55&statusID=4
	...
	{//show results
		...
		{//dbOut() function is a 'wrapper' designed to strip slashes, etc. of data leaving db
			...
			if(isset($stageID) && ($stageID >= 3)){$str .= '<a
				...
				href="' . VIRTUAL_PATH . 'library/char_powers.php?act=addTrait&catID='
												. $cat	        #combat-powers - combat-powers
			...
			if(isset($_SESSION['Privilege']) && ($_SESSION['Privilege'] >= 4)){$str .= '<a class="btn btn-info pull-right btn-xs" href="' . VIRTUAL_PATH .'library/char_powers.php?act=edit&powName=' . $powName . '&CatID=' . $cat . '" title="Click to edit"><span class="glyphicon glyphicon-edit"></span></a>';
			}
			$str .= '<br /><hr />';
		}
	}else{//no records
...
UPDATE `table_name`
 SET `field_name` = replace(same_field_name, 'unwanted_text', 'wanted_text')
	 ...
	 UPDATE `ma_CharPower`
		 SET `PowDesc` = replace(PowDesc, 'ne XXXcNameXXX‘s t', 'next'),
		 `PowDesc` = replace(PowDesc, 'XgenderX', 'XXXcGenderXXX');
*/

	#personalize data
	$str = customizeData($str, $cGen, $cName, 'n');
...
function genCategoryBtns($sql, $cName='', $str='' ){ #generate cat descs...
	#act/myAction already set on line #115
...
#http://localhost/WrDKv3/library/char_powers.php?act=physical-enhancement-powers&codeName=Havok&charID=26&statusID=4&gender=male
function addTrait(){
	...
	if(isset($_SESSION['Privilege']) && ($_SESSION['Privilege'] > 2)){
		...
		#http://localhost/WrDKv3/library/char_powers.php?act=AddTrait&catID=combat&powName=beserker&charID=60&codeName=Ulli&statusID=6
		...
		$cat      	= strip_tags($_GET['catID']);				#combat-powers - combat-powers
