...
switch ($myAction){//check 'act' for type of process
	case 'combat-advantages':
			...
			echo genCategoryBtns('combat advantages', $codeName);
			echo genCategory('combat advantages', $charID, $codeName, $gender, $stageID);
...
function genLegend($codeName = '', $charID = 0, $str='' ){ #generate category legend and links from array
	...
	$myLoadedQuery = [
			'combat-advantages', 'defensive-advantages', 'detection-advantages', 'faith-advantages', 'magical-advantages', 'mental-advantages', 'physical-advantages', 'restricted-advantages', 'social', 'other'
		];
		...
		if($act == $url){
				$str .= '<li class="active">
					<a style="color: white;" href="'
						. VIRTUAL_PATH . 'library/advantages.php?act='
						. $myLoadedQuery[$count++]
				...
				$str .= '<li>
					<a class="" href="' . VIRTUAL_PATH . 'library/advantages.php?'
						. '&codeName=' . $codeName
						. '&charID=' . $charID
						. '">'
						. ucwords($myLabel)
			...
			#show me unhighlighted all others
				...
				$str .= '<li>
					<a  href="' . VIRTUAL_PATH . 'library/advantages.php?act='
						. $myLoadedQuery[$count++]
						. '&codeName=' . $codeName
						. '&charID=' . $charID
						. '">'
...
function genCategory($sql, $charID, $codeName, $gender, $stageID,  $str='' ){ #generate cat descs...
	...
	{//show results
		...
		{//dbOut() function is a 'wrapper' designed to strip slashes, etc. of data leaving db
			...
			$str .= '<p><strong class="text-primary"><a name="' . $advantageLink . '">' . ucwords($advantageName) . '</a>: (#' . ucwords($skillID) . ')</strong> <br />'
				. dbOut($row['PowDesc']) . '</p>';
