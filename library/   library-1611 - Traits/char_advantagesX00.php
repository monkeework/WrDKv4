...
switch ($myAction){//check 'act' for type of process
	case 'combat-skills':
			echo genOverview($myAction, $aarContent['skillsCombat'], $codeName, $charID, $gender);
	...
	default:
		#customize category
			$myBody = "<p>
				...
				<p>
					Using the links from the power catorgys  you will find a great many descriptions of various advantages catagorized in to easily digestable bits. To view a catagory, either click on the links above to open one of the catagory containers, OR simply click on the 'plus' symbol attached to the catagory you wish to explore. Only one catagory can be open at a time. To close a catagory, click on another. Enjoy.
				</p>
				<p>
					<em class='text-muted'><b>Note</b> - Characters may routinely operate far below their shown scores, and sometimes far above those shown stats in extreme circumstances (with MOD approval). However, regardless of prior moderator approval, the player with the highest stated score (STAT/STAT CHAIN) always wins in a conflicted challenge.*</b></em>
				</p>
				<p>
				...
				<ol>
					<li>
						Please select between one and five advantages from the catagories listed below, copying those which are appropriate for your character from the provided list and pasting them in to your characters &#39;Advantages&#39; section.
					</li>
					<li>
						When pasting the descriptions, remember to <em> tailor</em>  the description to fit your character. Make them relevant to their history, use examples and your character&#39;s name, etc. Please try to make them unique and personable to the character they describe. Failure to personalize descriptions is one of the prime reasons for <em> rejected</em> character applications.
					</li>
					<li>
						Also, please proof read each and every description you place so that it is properly spell checked.
					</li>
					<li>
						Feel free to make up your own advantage if it fits the character. Such as &quot;Farm Work&quot; for Cannonball because he grew up on one. Or &quot;Mother Figure&quot; if your character likes taking care of others. In fact, we encourage to have at least one unique ability for your character.
					</li>
...
function genLegend($codeName = '', $charID = 0, $str='' ){ #generate category legend and links from array
	...
	$myLoadedQuery = [
			'combat-skills', 'defensive-skills', 'detection-skills', 'energy-control-skills', 'energy-emission-skills', 'faith-skills', 'mental-skills', 'restricted-skills'
		];

	$str .= '<h4><a href="' . THIS_PAGE . '">Skill Categories</a></h4>
	...
	foreach($myTitles as $title){
		...
		$act = str_replace("/WrDK/traits/skills.php?act=","", $chek );
		...
		if($act == $url){
				$str .= '<li class="active">
					<a style="color: white;" href="'
						. VIRTUAL_PATH . 'library/skills.php?act='
						. $myLoadedQuery[$count++]
				...
				$str .= '<li>
					<a class="" href="' . VIRTUAL_PATH . 'library/skills.php?'
						. '&codeName=' . $codeName
						. '&charID=' . $charID
						. '">'
						. ucwords($myLabel)
			...
			#show me unhighlighted all others
				...
				$str .= '<li>
					<a  href="' . VIRTUAL_PATH . 'library/skills.php?act='
						. $myLoadedQuery[$count++]
						. '&codeName=' . $codeName
						. '&charID=' . $charID
						. '">'
...
function genCategory($sql, $charID, $codeName, $gender, $stageID,  $str='' ){ #generate cat descs...
	...
	$sql = "SELECT SkillID, SkillName, SkillDesc, Available, LastUpdated  FROM `ma_CharSkill` WHERE CatID = '$cat'; ";
	...
	{//show results
		...
		{//dbOut() function is a 'wrapper' designed to strip slashes, etc. of data leaving db
			...
			$str .= '<p><strong class="text-primary"><a name="' . $skillLink . '">' . ucwords($skillName) . '</a>: (#' . ucwords($skillID) . ')</strong> <br />'
				. dbOut($row['PowDesc']) . '</p>';
			...
			if(isset($_SESSION['Privilege']) && ($_SESSION['Privilege'] >= 4)){$str .= '<a class="btn btn-info pull-right btn-xs" href="' . VIRTUAL_PATH .'library/skills.php?act=edit&powName=' . $skillName . '&CatID=' . $cat . '" title="Click to edit"><span class="glyphicon glyphicon-edit"></span></a>';
			}
			$str .= '<br /><hr />';
		}
	}else{//no records
