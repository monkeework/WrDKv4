	...
	if(mysqli_num_rows($result) > 0) {#records exist - process
		...
		while($row = mysqli_fetch_assoc($result)) {# process each row
			...
			if(!empty($row['StatusID']) && ($row['StatusID'] == $stageID)){
				...
				#https://www.google.com/search?q=aaron+taylor+johnson
				#https://www.google.com/search?q=' . $pbGoogle .'
				#replace whitespace, commas, and dashes with plus sign

				$pbGoogle      = preg_replace('/[ ,]+/', '+', trim($cName));
				$pbGoogleSearch = "<a href='https://www.google.com/search?q=marvel+super-hero+cinematic+universe+{$pbGoogle}+x-men+avengers' target='_blank'>Click to Google {$cName} here.</a>";
				...
				#http://localhost/git250-16q2/marvel-adventures//uploads/static---1.gif
				...
				#1. create thumbnail
				#$myImgPath = "../uploads/c{$cID}_thumb.jpg";
				$myImg = "<img class='thumbSM' src='{$myImgPath}' alt='{$cName} is
					currently {$stageID} - {$cID}' />";

				#2. create link
				$myPageLink = '<a href="profile.php?CodeName=' . $cName .
					'&id=' . $cID .
					'&act=show">' . $cName .
					'</a>';
				...
				echo "<p >{$myImg} <div>{$myPageLink} -  {$pbGoogleSearch}</div></p><hr />";
