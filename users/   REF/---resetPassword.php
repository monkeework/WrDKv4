...
{//Select administrator
	...
	$iConn = IDB::conn();
	$sql = "select AdminID,UserName,LastName,Email,Privilege,LastLogin,NumLogins from " . PREFIX . "Admin";
	if($_SESSION["Privilege"] == "admin")
...
function editDisplay()
{
	...
	get_header();
	$iConn = IDB::conn();
	$sql = sprintf("select AdminID,UserName,Email,Privilege from " . PREFIX . "Admin WHERE AdminID=%d",$myID);
	$result = @mysqli_query($iConn,$sql) or die(trigger_error(mysqli_error($iConn), E_USER_ERROR));
	if(mysqli_num_rows($result) > 0)//at least one record!
	{//show results
		while ($row = mysqli_fetch_array($result))
