...
function mk_btns4posts($priv='', $hID='', $uID='', $pQueryAdd='', $pQueryEdit='', $pQueryRemove='', $pQueryLock='', $tURL ='', $tOrder='', $str=''){
	...
	$str = '<!-- START buttons -->
		...
		if(empty($tOrder)){ $tOrder = 'DESC'; }

		$str .= '<a href="' . VIRTUAL_PATH . 'threads/index.php?' . $tUrl . '&tOrder=' . $tOrder . '"><span class="glyphicon glyphicon glyphicon-sort-by-attributes pull-right" aria-hidden="true" data-toggle="tooltip" title="Read Thread By Most Recent Post First"></span>
		</div></a>';
