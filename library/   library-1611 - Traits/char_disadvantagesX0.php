...
get_header('headerJumbo-inc.php', 'bgDisdvantage01.jpg', 'Flaws & Disdvantages');
...
switch ($myAction){//check 'act' for type of process
	case 'combat-advantages':
			echo genOverview($myAction, $aarLibrary_advantages['advantagesCombat'], $codeName, $charID, $gender);
	...
	case 'defensive-advantages':
			echo genOverview($myAction, $aarLibrary_advantages['advantagesDefense'], $codeName, $charID, $gender);
	...
	case 'detection-advantages':
			echo genOverview($myAction, $aarLibrary_advantages['advantagesDetection'], $codeName, $charID, $gender);
	...
	case 'faith-advantages':
			echo genOverview($myAction, $aarLibrary_advantages['advantagesFaith'], $codeName, $charID, $gender);
			echo genCategoryBtns('faith', $codeName);
			echo genCategory('faith', $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case 'magical-advantages':
			echo genOverview($myAction, $aarLibrary_advantages['advantagesMagical'], $codeName, $charID, $gender);
	...
	case 'mental-advantages':
			echo genOverview($myAction, $aarLibrary_advantages['advantagesMental'], $codeName, $charID, $gender);
	...
	case 'physical-advantages':
			echo genOverview($myAction, $aarLibrary_advantages['advantagesPhysical'], $codeName, $charID, $gender);
			echo genCategoryBtns('physical', $codeName);
			echo genCategory('physical', $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case 'restricted-advantages':
			echo genOverview($myAction, $aarLibrary_advantages['advantagesRestricted'], $codeName, $charID, $gender);
			echo genCategoryBtns('restricted', $codeName);
			echo genCategory('restricted', $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case 'social-advantages':
			echo genOverview($myAction, $aarLibrary_advantages['advantagesSocial'], $codeName, $charID, $gender);
			echo genCategoryBtns('social', $codeName);
			echo genCategory('social', $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case 'other-advantages':
			echo genOverview($myAction, $aarLibrary_advantages['advantagesOther'], $codeName, $charID, $gender);
	...
	default:
		#customize category
		$myBody = $aarLibrary_advantages['advantagesOverview'];
...
function genOverview($myAction = '', $mainContent = '', $cName, $charID, $cGen, $str=''){ #format descriptions from array and
	...
	$str .= getCusomizerForm($cName, $cGen);
	...
	$str .= customizeData($mainContent, $cGen, $cName, 'n');
...
function genLegend($codeName = '', $charID = 0, $str='' ){ #generate category legend and links from array
	...
	$myTitles = [ 'combat', 'defensive', 'detection', 'faith', 'magical*', 'mental', 'physical', 'restricted*', 'social', 'other' ];

	$myLoadedQuery = [ 'combat-advantages', 'defensive-advantages', 'detection-advantages', 'faith-advantages', 'magical-advantages', 'mental-advantages', 'physical-advantages', 'restricted-advantages', 'social-advantages', 'other-advantages' ];
	...
	foreach($myTitles as $title){
		...
		$act = str_replace("/WrDK/traits/char_disadvantages.php?act=","", $chek );
		...
		if($act == $url){
				$str .= '<li class="active">
					<a style="color: white;" href="'
						. VIRTUAL_PATH . 'library/char_disadvantages.php?act='
						. $myLoadedQuery[$count++]
				...
				$str .= '<li>
					<a class="" href="' . VIRTUAL_PATH . 'library/char_disadvantages.php?'
						. '&codeName=' . $codeName
						. '&charID=' . $charID
						. '&gender=' . $gender
						. '">'
			...
			#show me unhighlighted all others
				...
				$str .= '<li>
					<a  href="' . VIRTUAL_PATH . 'library/char_disadvantages.php?act='
						. $myLoadedQuery[$count++]
						. '&codeName=' . $codeName
						. '&charID=' . $charID
						. '&gender=' . $gender
...
function genCategory($sql, $charID, $cName, $cGen, $stageID,  $str='' ){ #generate cat descs...
	...
	{//show results
		...
		{//dbOut() function is a 'wrapper' designed to strip slashes, etc. of data leaving db
			...
			#$advantageDesc = customizeData($advantageDesc, $cGen, $cName, $stripTags='xx');
			...
			if(isset($_SESSION['Privilege']) && ($_SESSION['Privilege'] >= 4)){$str .= '<a class="btn btn-info pull-right btn-xs" href="' . VIRTUAL_PATH .'library/char_advantages.php?act=edit&powName=' . $advantageName . '&CatID=' . $cat . '" title="Click to edit"><span class="glyphicon glyphicon-edit"></span></a>';
			}
			$str .= '<br /><hr />';
		}
	}else{//no records
	...
	#personalize data
	$str = customizeData($str, $cGen, $cName, $stripTags='n');
