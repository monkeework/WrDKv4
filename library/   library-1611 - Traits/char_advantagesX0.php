...
get_header('headerJumbo-inc.php', 'bgAdvantage00.jpg', 'Merits & Advantages');
...
switch ($myAction){//check 'act' for type of process
	...
	case 'social-advantages':
			...
			echo genCategoryBtns($cat, $codeName);
			echo genCategory($sql, $sql, $cat, $charID, $codeName, $gender, $stageID);
...
function genOverview($myAction = '', $content = '', $cName, $charID, $cGen, $str=''){ #format descriptions from array and
...
function genLegend($codeName = '', $charID = 0, $gender='', $str='' ){ #generate category legend and links from array
	...
	$titles = [
			'combat', 'defensive', 'detection', 'faith', 'magical*', 'mental', 'physical', 'restricted*', 'social', 'other'
		];
	...
	foreach($titles as $title){
		...
		$act = str_replace("/WrDK/traits/char_advantages.php?act=","", $chek );
...
#echo genCategorymk_customCat($sql, $cat, $charID, $codeName, $gender, $stageID);
...
function genCategory($sql, $cat, $charID, $cName, $cGen, $stageID, $str='' ){ #generate cat descs...
	...
	{//show results
		...
		{//dbOut() function is a 'wrapper' designed to strip slashes, etc. of data leaving db
			...
			$advantageDesc = customizeData($advantageDesc, $cGen, $cName, $stripTags='x');
			...
			$str .= '<p><strong class="text-primary"><a name="' . $advantageLink . '">' . ucwords($advantageName) . '</a>: (#' . ucwords($advantageID) . ')</strong> <br />' . $advantageDesc . '</p>';
