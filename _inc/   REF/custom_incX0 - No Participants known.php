...
function mk_cFeaturedLinks($threadID, $aPostIndex, $aCharIndex=[], $aParticipants=[], $str='') {

	if(empty($threadID)){

		return 'No Participants yet listed';
		break;
	}
	#dumpDie($threadID);

	/**
		 * PATCH APPLIED TO DEAL WITH ERROR OF NO 'charIndex'
		 * @todo resolve error correctly
		 */
	if(isset($_SESSION['charIndex'])){ $aCharIndex=$_SESSION['charIndex']; }
		/*
			$aPostIndex	 ==	  array (size=1)
														0 =>
																array (size=54)
																		0 =>
																				array (size=4)
			...
			compare again....
			$_SESSION['charIndex']);  ==  1 => string 'Cyclops'
			$aCharIndex               ==  1 => string 'Cyclops'
		*/


	/**
	 * Array 1 (aPostIndex) has the deets regarding each post in a thread/all threads
	 * Array 2 ($charIndex) has the ddets on all characters namess key (id) => value (codename)
	 *
	 * Objective => return a string of codenames,  ordered by first occurance for each thread
	 * IE : <a href="charId-1" > Cyclops</a>, <a href="charId-3" > Wiccan</a>, <a href="charId-23" > Psyche</a>...
	 *
	 * char's only appear once, appear based on firs posting to an dividual thread.
	 * we have a string of names for each thread.
	 *
	 *
	 ...
	 *
	 *
	 # arr 1 => post values to sort 	=> $aPostIndex
	 # turns - TheadID   -> int
	 *			 - PostId    -> int
	 *			 - CharId    -> int
	 *			 - PostOrder -> int
	 *
	 * HAVE THIS                    	=> aCharNameIndex
	 #			 - 1 => string 'Cyclops' (length=7)
	 #			 - 2 => string 'Chimaera' (length=8)
	 *			 - 3 => string 'Wiccan' (length=6)
	 * HAVE THIS
	 *
	 * pull compared values from
	 * NEED THIS
	 * @TODO make second array
	 **/

	#MAKE index array to pull value comparsions from...
	#MAKE array of particpants

	//foreach($aPostIndex[0][]['TheadID'] as $tID) {

	foreach($aPostIndex[0] as $arrData) {

		if($arrData['ThreadID'] == $threadID) {
			#make character id
			$chk_charID = $arrData['CharID'];

			var_dump($chk_charID);


			//$charID['CharID'] = $aPostIndex['CharID']; #get character id
			if($chk_charID == 0){
					continue;
			}else{
				$aParticipants = $charID['CharID'];
	...
	if($data['ThreadID'] == $threadID) {
			#make character id
			$charID = $data['CharID']; #get character id

			if($charID > 0){


				#convert charater int to string name of character
				$aCharName[] = $_SESSION['charIndex'][$charID];
...
#var_dump($aCharNameIndex);
				...
				$str .= '<a href="./../characters/profile.php?CodeName=Chimaera&id=2&act=show" title="" >Chimeara</a>
