...
function maxDoc_library_char_aptitudes(){
	/**
...
get_header('headerJumbo-inc.php', 'bgSkills00.jpg', 'Aptitudes &amp; Skills');
...
switch ($myAction){//check 'act' for type of process
	...
	case 'alternate-sciences':
			echo genOverview($myAction, $aarLibrary_aptitudes['alternate-sciences'], $codeName, $charID, $gender);
			echo genCategoryBtns('alternate sciences', $codeName);
			echo genCategory('alternate sciences', $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case 'astronomy':
			echo genOverview($myAction, $aarLibrary_aptitudes['astronomy'], $codeName, $charID, $gender);
			echo genCategoryBtns('astronomy', $codeName);
			echo genCategory('astronomy', $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case "biology":
			echo genOverview($myAction, $aarLibrary_aptitudes['biology'], $codeName, $charID, $gender);
			echo genCategoryBtns('biology', $codeName);
			echo genCategory('biology', $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case "chemistry":
			echo genOverview($myAction, $aarLibrary_aptitudes['chemistry'], $codeName, $charID, $gender);
			echo genCategoryBtns('chemistry', $codeName);
			echo genCategory('chemistry', $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case "crime":
			echo genOverview($myAction, $aarLibrary_aptitudes['crime'], $codeName, $charID, $gender);
			echo genCategoryBtns('crime', $codeName);
			echo genCategory('crime', $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case "crime-enforcement":
			echo genOverview($myAction, $aarLibrary_aptitudes['crime-enforcement'], $codeName, $charID, $gender);
			echo genCategoryBtns('crime enforcement', $codeName);
			echo genCategory('crime enforcement', $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case "combat":
			echo genOverview($myAction, $aarLibrary_aptitudes['combat'], $codeName, $charID, $gender);
	...
	########################################################
	case "law":
			echo genOverview($myAction, $aarLibrary_aptitudes['law'], $codeName, $charID, $gender);
			echo genCategoryBtns('law', $codeName);
			echo genCategory('law', $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case "cognitive-sciences-humanities":
			echo genOverview($myAction, $aarLibrary_aptitudes['cognitive-sciences-humanities'], $codeName, $charID, $gender);
			echo genCategoryBtns('cognitive sciences & humanities', $codeName);
			echo genCategory('cognitive sciences & humanities', $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case "computer-sciences":
			echo genOverview($myAction, $aarLibrary_aptitudes['computer-sciences'], $codeName, $charID, $gender);
			echo genCategoryBtns('computer sciences', $codeName);
			echo genCategory('computer sciences', $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case "earth-sciences":
			echo genOverview($myAction, $aarLibrary_aptitudes['earth-sciences'], $codeName, $charID, $gender);
			echo genCategoryBtns('earth sciences', $codeName);
			echo genCategory('earth sciences', $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case "engineering":
			echo genOverview($myAction, $aarLibrary_aptitudes['engineering'], $codeName, $charID, $gender);
			echo genCategoryBtns('engineering', $codeName);
			echo genCategory('engineering', $charID, $codeName, $gender, $stageID);
	...
	########################################################
	case "faith":
			echo genOverview($myAction, $aarLibrary_aptitudes['faith'], $codeName, $charID, $gender);
	...
	########################################################
	case "medicine":
			echo genOverview($myAction, $aarLibrary_aptitudes['famedicineith'], $codeName, $charID, $gender);
			echo genCategoryBtns('medicine', $codeName);
			echo genCategory('medicine', $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case "mental":
			echo genOverview($myAction, $aarLibrary_aptitudes['mental'], $codeName, $charID, $gender);
			echo genCategoryBtns('mental', $codeName);
			echo genCategory('mental', $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case "occult":
			echo genOverview($myAction, $aarLibrary_aptitudes['occult'], $codeName, $charID, $gender);
			echo genCategoryBtns('occult', $codeName);
			echo genCategory('occult', $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case "physics":
			echo genOverview($myAction, $aarLibrary_aptitudes['physics'], $codeName, $charID, $gender);
			echo genCategoryBtns('physics', $codeName);
			echo genCategory('physics', $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case "piloting":
			echo genOverview($myAction, $aarLibrary_aptitudes['piloting'], $codeName, $charID, $gender);
			echo genCategoryBtns('piloting', $codeName);
			echo genCategory('piloting', $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case "weapons":
			echo genOverview($myAction, $aarLibrary_aptitudes['weapons'], $codeName, $charID, $gender);
			echo genCategoryBtns('weapons', $codeName);
			echo genCategory('weapons', $charID, $codeName, $gender, $stageID);

		break;
	########################################################
	case "other":
			echo genOverview($myAction, $aarLibrary_aptitudes['other'], $codeName, $charID, $gender);
			echo genCategoryBtns('other', $codeName);
			echo genCategory('other', $charID, $codeName, $gender, $stageID);
...
*/
	...
	default:
		#customize category
		echo genOverview($myAction, $aarLibrary_aptitudes['overview-aptitudes'], $codeName, $gender, $charID);;#END default
...
function genLegend($codeName='', $charID = 0, $str='' ){ #generate category legend and links from array
	...
	$myTitles = [
			'alternate sciences', 'astronomy', 'biology', 'chemistry', 'crime', 'crime enforcement', 'combat', 'law', 'cognitive sciences & humanities', 'computer sciences', 'earth sciences', 'engineering', 'faith', 'medicine', 'mental', 'occult', 'physics', 'piloting', 'weapons', 'other'
		];
	...
	$str .= '<h4><a href="' . THIS_PAGE . '">Aptitudes & Skills</a></h4>
	...
	foreach($myTitles as $title){
		...
		$act = str_replace("/WrDK/traits/char_aptitudes.php?act=","", $chek );
		...
		if($act == $url){
				$str .= '<li class="active">
					<a style="color: white;" href="'
						. VIRTUAL_PATH . 'library/char_aptitudes.php?act='
						. $myLoadedQuery[$count++]
				...
				$str .= '<li>
					<a class="" href="' . VIRTUAL_PATH . 'library/char_aptitudes.php?'
						. '&codeName=' . $codeName
						. '&charID=' . $charID
						. '&gender=' . $gender
						. '">'
			...
			#show me unhighlighted all others
				...
				$str .= '<li>
					<a  href="' . VIRTUAL_PATH . 'library/char_aptitudes.php?act='
						. $myLoadedQuery[$count++]
						. '&codeName=' . $codeName
						. '&charID=' . $charID
						. '&gender=' . $gender
...
function genCategory($cat, $charID, $codeName, $gender, $stageID, $sql='', $str='' ){ #generate cat descs...
	#act/myAction already set on line #115
	#$cat = $sql; //for btns

	$sql = "SELECT SkillID, SkillName, SkillDesc, Available, LastUpdated  FROM `ma_CharSkills` WHERE CatID = '$cat'; ";
	...
	{//show results
		...
		{//dbOut() function is a 'wrapper' designed to strip slashes, etc. of data leaving db
			$skillID   = (int)dbOut($row['SkillID']);
			$skillName = dbOut($row['SkillName']);
			$skillDesc = dbOut($row['SkillDesc']);

			$skillLink = str_replace('', ' ', $skillName);

			#uppercase everything up to first paren, then "small"
			#creat anchors - see: http://www.echoecho.com/htmllinks08.htm
			$str .= '<p><strong class="text-primary"><a name="' . $skillLink . '">' . ucwords($skillName) . '</a>: (#' . ucwords($skillID) . ')</strong> <br />' . dbOut($row['SkillDesc']) . '</p>';


			if(isset($stageID) && ($stageID >= 3)){$str .= '<a
				class="btn btn-info pull-left btn-xs"
				href="' . VIRTUAL_PATH . 'library/char_aptitudes.php?act=addTrait&catID='
												. $cat	        #combat-skills - combat-skills
				. '&skillID='   . $skillID  		#1             - beserker	id num
				. '&skillName=' . $skillName    #name of trait - beserker
				. '&charID=' 		. $charID       #character id  - 60
				. '&codeName='  . $codeName     #codename      - Ully
				. '&stageID='   . $stageID			#3-6 = can add a power, else nope
				. '"
				title="Click to add ' . $skillName . '  to ' . $codeName . '"><span class="glyphicon glyphicon-plus"></span> Add ' . strtoupper($skillName) . ' power to ' . $codeName . '</a>';
			}
			...
			if(isset($_SESSION['Privilege']) && ($_SESSION['Privilege'] >= 4)){$str .= '<a class="btn btn-info pull-right btn-xs" href="' . VIRTUAL_PATH .'library/char_aptitudes.php?act=edit&powName=' . $skillName . '&CatID=' . $cat . '" title="Click to edit"><span class="glyphicon glyphicon-edit"></span></a>';
			}
			$str .= '<br /><hr />';
		}
	}else{//no records
...
function genCategoryBtns($sql, $codeName='', $str='' ){ #generate cat descs...
	...
	$sql = "SELECT SkillName, Available FROM `ma_CharSkills` WHERE CatID = '$sql'; ";
	...
	{//show results
		...
		{//dbOut() function is a 'wrapper' designed to strip slashes, etc. of data leaving db
			...
			$skillName  = dbOut($row['SkillName']);
			$skillAvail = dbOut($row['Available']);

			$skillLink  = str_replace ('-', ' ', $skillName);

			#uppercase everything up to first paren, then "small"
			#creat anchors - see: http://www.echoecho.com/htmllinks08.htm
			$str .= '<small><a class="btn btn-primary btn-xs" href="#' . $skillLink . '">' . ucwords($skillName) . '</a></small>';
		}
...
#dumpDie($_GET);
		...
		#get prior power description
		$sql = "SELECT `SkillID`, `SkillName`, `SkillDesc` FROM ma_CharPower WHERE PowID = '$skillID';";
...
#string 'Mary-Sue-ism' (length=12)
		...
		';
			...
			{//dbOut() function - 'wrapper' to strip slashes, etc. of data leaving db
				...
				echo '<h3 class="text-center">Edit ' . ucwords($skillName) . ' power entry</h3>
...
#HELPER FUNCTIONS
#strPersonalizering($myDesc, $codename, $gender);
